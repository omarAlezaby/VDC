...
def optimize_inversion(ref_img, ref_latent, z_enc, t_enc, init_cond, sampler, itrs, uc):
    ...
    adam = torch.optim.AdamW(params = [z_enc], lr = .005)
...
def cond_mapping_perstep_mlp(ref_imgs, ref_latents, latents_z, t_enc, init_cond, sampler, itrs, scale, uc, batch_size=2):
    ...
    for iii in pbar:
        ...
        for ba in range(batch_size):
            data_num = random.sample(range(len(ref_latents)),1)[0]
            ...
            # Condition Steering
            samples = sampler.decode_VDC(latents_z[data_num], uc, t_enc, unconditional_guidance_scale=scale,
                                        unconditional_conditioning=uc, perstep_cond=out_conds)
            
            # Condition Optimization
            x_samples = sampler.model.decode_first_stage(samples)
            loss = loss_fn(samples, ref_latents[data_num], reduction = 'mean')/batch_size
            loss += loss_fn(x_samples, ref_imgs[data_num], reduction = 'mean')/batch_size
...
def main():
    ...
    parser.add_argument(
        "--cond_path",
        type=str,
        default="outputs/rain/learned_conds.npy",
    ...
    shutil.copy(f"scripts/CondMapping.py", f"{outpath}/run_script.py")
    ...
    # assert os.path.isdir(opt.input_imgs)
    if opt.cond_path is None:
        ...
        if os.path.isdir(opt.input_imgs):
            ...
            ref_names = [opt.reference_imgs]

        logging.info(f"Condition Optimization Examples: {ref_names}")


        for fi, ref_fi in zip(imgs_names, ref_names):
            inp_image = load_img(fi).to(device)
            ref_image = load_img(ref_fi).to(device)
            
            inp_image = repeat(inp_image, '1 ... -> b ...', b=1)
            inp_latent = model.get_first_stage_encoding(model.encode_first_stage(inp_image))  # move to latent space
            inp_latents.append(inp_latent)

            ref_image = repeat(ref_image, '1 ... -> b ...', b=1)
            ref_imgs.append(ref_image.clone())
            ref_latent = model.get_first_stage_encoding(model.encode_first_stage(ref_image))  # move to latent space
        ...
        with model.ema_scope():
            ...
            if opt.cond_path is None:
                ...
                with torch.no_grad():
                    ...
                    enc_inp_latents = []
                    for i in range(len(inp_latents)):
                        enc_inp_latents.append(sampler.ddim_inverse(inp_latents[i], uc, t_enc)[-1].detach())

                c_out_new = cond_mapping_perstep_mlp(ref_imgs, ref_latents, enc_inp_latents, t_enc, uc, sampler, opt.opt_cond_itrs, opt.scale, uc, batch_size=opt.batch_size)


                if opt.save_cond:
                    logging.info(torch.cat(c_out_new).detach().cpu().numpy().shape)
